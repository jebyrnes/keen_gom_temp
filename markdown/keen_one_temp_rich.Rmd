...
```{r}
keen_sl_mod <-  glmer(S_LATISSIMA_SQ_M +1e-5~ group_temp + temp_group_cent + 
                       PERCENT_ROCK + (1|SITE),
                     data = keen_one,
                     family=gaussian(link="log"),
                     nAGQ=0)
...
diff_mod <- lme(SL_PERC_DIFF ~ TEMP_DIFF + SITE_PERCENT_ROCK, 
                random = (~1|SITE), 
...
Here again we see that for one degree change we have a `r round(fixef(diff_mod), 3)`% change in kelp.
...
```{r, fig.cap="<b>NOTE: I'm futzing with igraph to see if I can automate this</b>",
...
rich_diff_mod <- lme(RICH_PERC_DIFF ~ TEMP_DIFF+SL_DIFF,
                        random = (~1|SITE), 
                      data = difference_data,
                      na.action = na.omit)
